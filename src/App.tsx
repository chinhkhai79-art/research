...
export default function App() {
  ...
  const normalizeHunterFilterConfig = (cfg: YouTubeConfig): YouTubeConfig => {
    ...
    const rawRegions = Array.isArray(cfg.regions) ? cfg.regions.map(code => String(code || '').toUpperCase()) : [];
    const regions = rawRegions.includes('ALL')
      ? ['ALL']
      : Array.from(new Set(rawRegions.filter(code => allowedRegions.has(code))));
    if (!regions.length) {
      const legacyRegion = String(cfg.region || 'VN').toUpperCase();
      regions.push(allowedRegions.has(legacyRegion) ? legacyRegion : 'VN');
    }
    const primaryRegion = regions.includes('ALL')
      ? (allowedRegions.has(String(cfg.region || '').toUpperCase()) ? String(cfg.region).toUpperCase() : 'VN')
      : regions[0];
    ...
    return {
      ...
      publishedAfter: cfg.publishedAfter || 'month',
      minSub,
      maxSub,
      minVideo,
      maxVideo,
...
JSON mẫu:
{
    ...
    const fetchSuggestionVideos = async (publishedAfter?: string) => {
      ...
      for (const seed of seedList) {
        const res = await limitedYoutubeFetch('search', {
          ...
          type: 'video',
          regionCode,
          relevanceLanguage: (REGION_YT_CONFIG as any)[regionCode]?.relevanceLanguage || undefined,
          publishedAfter,
          maxResults: 10,
          order: 'viewCount'
        });
        ...
        for (const seed of searchSeeds) {
          ...
          try {
            const res = await youtubeFetch('search', {
              ...
              type: 'video',
              regionCode: nicheRegion,
              relevanceLanguage: (REGION_YT_CONFIG as any)[nicheRegion]?.relevanceLanguage || undefined,
              publishedAfter,
              maxResults: 15,
              order: 'date',
              videoDuration: 'short'
  ...
  const fetchRealKeywordIdeas = async (base: string, regionCode?: string, publishedAfter?: string): Promise<KeywordIdea[]> => {
    ...
    const addCandidate = (value: string, source: 'base' | 'autocomplete' | 'title' | 'tag', bonus = 0, views = 0, vph = 0, likeRate = 0, sampleTitle = '', localHit = 0) => {
      ...
      const relevance = source === 'base' ? 100 : getKeywordRelevanceScore(cleaned, baseText, source);
    ...
    try {
      ...
      if (candidateMap.size < 20) {
        ...
        const autocomplete = await fetchYoutubeAutocompleteSuggestions(baseText, primaryKeywordRegion);
...
Trong đó id phải đúng với candidate đã cung cấp.`;
  ...
  const buildKeywordIdeasForAutoSwitch = (keywords: string[], mainKeyword: string): KeywordIdea[] => {
    ...
    const cleaned = keywords
      ...
      .filter((text, idx) => idx === 0 || getKeywordRelevanceScore(text, mainKeyword, 'autocomplete') >= getKeywordRelevanceThreshold('autocomplete'));
    ...
    return cleaned.slice(0, 20).map((text, idx) => {
      ...
      const relevance = isMain ? 100 : getKeywordRelevanceScore(text, mainKeyword, 'autocomplete');
      const wordCount = getKeywordTokens(text).length;
      const competition = isMain ? 'Trung bình' : wordCount <= 2 ? 'Cao' : wordCount <= 4 ? 'Trung bình' : 'Thấp';
      const scoreValue = isMain ? 10 : Math.max(4.5, Math.min(9.8, relevance / 10));
      return {
        text,
        competition,
        score: isMain ? '10/10' : `${scoreValue.toFixed(1)}/10`,
        status: 'idle' as const
      };
    });
  };
  ...
  // Bộ lọc liên quan theo token: giữ chặt chủ đề gốc thay vì lấy tag YouTube thô.
  // Ví dụ "video AI" cho phép "text to video ai", "ai video generator", "ai content creation";
  // nhưng loại "phim hay", "phim ngắn Trung Quốc", "lồng tiếng Việt" vì không có token chủ đề AI.
  const KEYWORD_LOW_SIGNAL_TOKENS = new Set([
    'video', 'videos', 'youtube', 'short', 'shorts', 'clip', 'clips', 'channel', 'official',
    'new', 'latest', 'best', 'top', 'full', 'part', 'review', 'reviews', 'free', '2025', '2026',
    'phim', 'film', 'movie', 'movies', 'kenh'
  ]);

  const normalizeKeywordMatchText = (value: string) =>
    String(value || '')
      .toLowerCase()
      .normalize('NFD')
      .replace(/[\u0300-\u036f]/g, '')
  ...
  const getKeywordTokens = (value: string) =>
    normalizeKeywordMatchText(value)
      .split(' ')
      .map(token => token.trim())
      .filter(token => token.length >= 2 && !/^\d+$/.test(token));

  const getKeywordRelevanceThreshold = (source: string) => {
    if (source === 'autocomplete') return 58;
    if (source === 'title') return 66;
    if (source === 'tag') return 72;
    return 64;
  };

  const getKeywordRelevanceScore = (candidate: string, baseKeyword: string, source: string = 'title') => {
    const base = normalizeKeywordMatchText(baseKeyword);
    const cand = normalizeKeywordMatchText(candidate);
    if (!base || !cand) return 0;
    if (base === cand) return 100;
    if (cand.length < 2 || cand.length > 80) return 0;

    const baseTokens = Array.from(new Set(getKeywordTokens(base)));
    const candidateTokens = Array.from(new Set(getKeywordTokens(cand)));
    if (!baseTokens.length || !candidateTokens.length) return 0;

    const candidateSet = new Set(candidateTokens);
    const overlapTokens = baseTokens.filter(token => candidateSet.has(token));
    const distinctBaseTokens = baseTokens.filter(token => !KEYWORD_LOW_SIGNAL_TOKENS.has(token));
    const effectiveDistinct = distinctBaseTokens.length ? distinctBaseTokens : baseTokens;
    const distinctOverlap = effectiveDistinct.filter(token => candidateSet.has(token));
    const overlapRatio = overlapTokens.length / Math.max(1, baseTokens.length);
    const distinctRatio = distinctOverlap.length / Math.max(1, effectiveDistinct.length);
    const containsAllBaseTokens = baseTokens.every(token => candidateSet.has(token));
    const containsBasePhrase = cand.includes(base) || base.includes(cand);

    // Nếu seed có token phân biệt (ví dụ AI trong "video AI"), không chấp nhận ứng viên chỉ trùng token chung như "video".
    if (distinctBaseTokens.length && distinctOverlap.length === 0) return 0;
    if (!overlapTokens.length) return 0;

    let score = 0;
    if (containsAllBaseTokens) score += 74;
    else score += 36 * overlapRatio + 38 * distinctRatio;
    if (containsBasePhrase) score += 14;

    if (source === 'autocomplete') score += 12;
    else if (source === 'title') score += 5;
    else if (source === 'tag') score += 2;

    if (candidateTokens.length >= 3 && candidateTokens.length <= 6) score += 5;
    if (candidateTokens.length === 1 && baseTokens.length > 1) score -= 7;

    return Math.max(0, Math.min(100, Math.round(score)));
  };

  const fetchYoutubeAutocompleteSuggestions = async (baseKeyword: string, regionCode?: string): Promise<string[]> => {
    const region = String(regionCode || config.region || 'VN').toUpperCase();
    const language = (REGION_YT_CONFIG as any)[region]?.relevanceLanguage || (region === 'VN' ? 'vi' : 'en');
    const params = new URLSearchParams({ q: baseKeyword, hl: language, region });

    try {
      const response = await fetch(`/api/youtube-suggest?${params.toString()}`, {
        signal: abortControllerRef.current?.signal
      });
      if (response.ok) {
        const data = await response.json().catch(() => ({}));
        if (Array.isArray(data?.suggestions)) {
          return Array.from(new Set(data.suggestions.map((x: any) => String(x || '').trim()).filter(Boolean)));
        }
      }
    } catch (error: any) {
      if (error?.name === 'AbortError') throw error;
      console.warn('youtube-suggest proxy error:', error);
    }

    try {
      const directUrl = `https://suggestqueries.google.com/complete/search?client=firefox&ds=yt&q=${encodeURIComponent(baseKeyword)}&hl=${encodeURIComponent(language)}`;
      const response = await fetch(directUrl, { signal: abortControllerRef.current?.signal });
      if (!response.ok) return [];
      const data = await response.json();
      return Array.isArray(data?.[1]) ? Array.from(new Set(data[1].map((x: any) => String(x || '').trim()).filter(Boolean))) : [];
    } catch (error: any) {
      if (error?.name === 'AbortError') throw error;
      return [];
    }
  ...
  const buildKeywordPhrasesFromTitle = (title: string, baseKeyword: string) => {
    ...
    // Không còn fallback lấy 4-5 từ đầu tiêu đề vì đây là nguồn gây ra "phim hay", "lồng tiếng Việt"... lạc chủ đề.
    for (let size = 2; size <= 6; size++) {
      for (let i = 0; i <= words.length - size; i++) {
        const phrase = words.slice(i, i + size).join(' ').trim();
        if (getKeywordRelevanceScore(phrase, baseKeyword, 'title') >= getKeywordRelevanceThreshold('title')) {
          phrases.push(phrase);
        }
      }
    }
  ...
  const fetchYoutubeV3RelatedKeywordsForAutoSwitch = async (baseKeyword: string, regionCode: string, publishedAfter?: string) => {
    ...
    const addKeyword = (text: string, source: 'base' | 'autocomplete' | 'title' | 'tag', bonus = 0, views = 0, vph = 0, likeRate = 0, sampleTitle = '', localHit = 0) => {
      ...
      const relevance = source === 'base' ? 100 : getKeywordRelevanceScore(cleaned, baseKeyword, source);
      if (source !== 'base' && relevance < getKeywordRelevanceThreshold(source)) return;
  ...
  const getSelectedHunterRegions = (cfg: YouTubeConfig): string[] => {
    const allowed = new Set(REGIONS.map(item => item.code));
    const configured = Array.isArray(cfg.regions) ? cfg.regions.map(code => String(code || '').toUpperCase()) : [];

    if (configured.includes('ALL')) return REGIONS.map(item => item.code);
    ...
    const legacyRegion = String(cfg.region || '').toUpperCase();
    if (!selected.length && allowed.has(legacyRegion)) selected.push(legacyRegion);
    return selected.length ? selected : ['VN'];
  };

  const getPrimaryHunterRegion = (cfg: YouTubeConfig): string => {
    const configured = Array.isArray(cfg.regions) ? cfg.regions : [];
    if (configured.includes('ALL')) {
      const legacyRegion = String(cfg.region || '').toUpperCase();
      return REGIONS.some(item => item.code === legacyRegion) ? legacyRegion : 'VN';
    }
    return getSelectedHunterRegions(cfg)[0] || 'VN';
  };

  const getPrioritizedHunterRegionPlan = (cfg: YouTubeConfig, allowFallback: boolean) => {
    const selectedRegions = getSelectedHunterRegions(cfg);
    const allSelected = Array.isArray(cfg.regions) && cfg.regions.includes('ALL');
    const selectedSet = new Set(selectedRegions);
    const preferred = selectedRegions.map(code => ({ code, selected: true }));
    if (!allowFallback || allSelected) return preferred;
  ...
  const localizeHunterKeywordForRegion = (rawKeyword: string, regionCode: string) => {
    if (!rawKeyword) return '';
    if (!regionCode || regionCode === 'VN') return rawKeyword;
    return translateKeywordSimple(rawKeyword, getLanguageForRegion(regionCode));
  };
  ...
  const startHunter = async () => {
    ...
    try {
      ...
      const allowRegionFallback = isAutoHunt || shouldAutoSwitchKeywords;
      const regionPlan = getPrioritizedHunterRegionPlan(hunterConfig, allowRegionFallback);
      const primaryRegion = regionPlan[0]?.code || getPrimaryHunterRegion(hunterConfig);
      const effectivePublishedAfter = hunterConfig.deepDrillSmallTrend ? 'month' : (hunterConfig.publishedAfter || 'month');
      ...
      let primaryKeywordIdeasSet = false;
      let usedRegionalFallback = false;

      for (let regionIndex = 0; regionIndex < regionPlan.length; regionIndex++) {
        if (!isHuntingRef.current || resultsRef.current.length >= STOP_LIMIT) break;

        const regionStep = regionPlan[regionIndex];
        const currentRegion = regionStep.code;
        const regionCfg = REGION_YT_CONFIG[currentRegion] || REGION_YT_CONFIG.VN;
        const regionName = REGIONS.find(item => item.code === currentRegion)?.name || currentRegion;
        const regionTag = ` [QG: ${currentRegion}]`;
        const regionStage = regionStep.selected ? 'khu vực đã chọn' : 'khu vực bổ sung';
        if (!regionStep.selected) usedRegionalFallback = true;

        let scanKeywords: string[] = [];
        if (isAutoHunt) {
          const regionalSeeds = diversifySeedsByTopic(shuffleList(getAutoHuntSeeds(currentRegion)));
          scanKeywords = regionalSeeds.slice(0, regionStep.selected ? 16 : 6);
        } else {
          const localizedKeyword = localizeHunterKeywordForRegion(rawKeyword, currentRegion);
          const baseKeywords = Array.from(new Set([localizedKeyword, rawKeyword].map(k => String(k || '').trim()).filter(Boolean)));

          if (shouldAutoSwitchKeywords) {
            setStatus(regionStep.selected
            ...
            scanKeywords = [localizedKeyword || rawKeyword];
          }
        }

        if (!scanKeywords.length) continue;
        ...
        for (let k = 0; k < scanKeywords.length; k++) {
          ...
          setStatus(
            isAutoHunt
              ? `${regionStep.selected ? 'Ưu tiên' : 'Bổ sung'} ${regionName}: cụm "${searchKeyword}" (${resultsRef.current.length}/${STOP_LIMIT})`
              : autoSwitchMode
                ? `${regionStep.selected ? 'Ưu tiên' : 'Bổ sung'} ${regionName} · YouTube V3: "${searchKeyword}" (${k + 1}/${scanKeywords.length}) — đã có ${resultsRef.current.length}/${STOP_LIMIT} kênh`
        ...
        for (const candidate of candidates as any[]) {
          ...
          const autoSeedKey = `${currentRegion}:${normalizeHunterKeyword(searchKeyword).slice(0, 48)}`;
      ...
      if (resultsRef.current.length >= STOP_LIMIT) {
        ...
        setStatus(usedRegionalFallback
          ? `Hoàn tất: đã quét khu vực đã chọn trước rồi mới mở rộng khu vực khác. Tìm được ${resultsRef.current.length} kênh.`
          : `Hoàn tất quét khu vực đã chọn. Tìm được ${resultsRef.current.length} kênh.`);
      } else if (hunterConfig.autoNiche) {
        setStatus(usedRegionalFallback
          ? `Đã quét hết từ khóa khu vực ưu tiên rồi mới bổ sung khu vực khác bằng YouTube API V3. Tìm được ${resultsRef.current.length}/${STOP_LIMIT} kênh đạt điều kiện.`
          : `Đã quét hết từ khóa liên quan tại khu vực đã chọn bằng YouTube API V3. Tìm được ${resultsRef.current.length}/${STOP_LIMIT} kênh đạt điều kiện.`);
      } else {
  ...
  const toggleRegion = (code: string) => {
    setConfig(prev => {
      ...
      const nextPrimaryRegion = nextRegions.includes('ALL')
        ? (REGIONS.some(item => item.code === prev.region) ? prev.region : 'VN')
        : (nextRegions[0] || 'VN');
      return { ...prev, regions: nextRegions, region: nextPrimaryRegion };
    });
  };
                ...
                <div className="vtw-filter-grid vtw-main-search-card grid grid-cols-12 gap-4 bg-[#f1f1f1] p-4 border border-[#bbb] rounded shadow-sm">
                  <div className="vtw-filter-fields col-span-12 lg:col-span-9 grid grid-cols-3 gap-6">
                    <div className="space-y-2">
                      ...
                      <div className="flex items-center gap-2"><div className="w-1/3 text-right text-[11px] font-bold text-[#2c3e50]">Đăng trong:</div><div className="w-2/3 border border-[#999] bg-white h-7 px-2 flex items-center justify-between">Tháng này <span>▼</span></div></div>
                    </div>
            ...
            >
              ...
              <div className="vtw-api-modal-content p-5 space-y-5 overflow-y-auto custom-scrollbar max-h-[65vh]">
                ...
                <div className="bg-indigo-50/50 p-4 rounded-3xl border-2 border-indigo-100/50 shadow-sm relative overflow-hidden group">
                  ...
                  <div className="flex items-center gap-2 mb-3 relative z-10 flex-wrap md:flex-nowrap">
                    ...
                    <div className="ml-auto flex flex-nowrap items-center justify-end gap-2 shrink-0">
                      <a href="https://aistudio.google.com/app/apikey" target="_blank" rel="noopener noreferrer" className="inline-flex items-center justify-center gap-1 rounded-xl border border-indigo-200 bg-white px-2.5 py-2 text-[9px] font-bold text-indigo-600 shadow-sm hover:bg-indigo-100 transition-all active:scale-95 whitespace-nowrap shrink-0">
                        Lấy Key Gemini <ExternalLink size={11} />
                      </a>
                      <button type="button" onClick={openKeyGuideVideo} className="inline-flex items-center justify-center gap-1 rounded-xl border border-indigo-600 bg-indigo-600 px-2.5 py-2 text-[9px] font-bold text-white shadow-sm hover:bg-indigo-700 transition-all active:scale-95 whitespace-nowrap shrink-0">
                ...
                <div className="bg-red-50/30 p-6 rounded-3xl border-2 border-red-100/50 shadow-sm relative overflow-hidden group">
                  ...
                  <div className="flex items-center gap-2 mb-4 relative z-10 flex-wrap md:flex-nowrap">
                    <div className="p-2 bg-red-600 rounded-xl text-white shadow-lg shrink-0">
                      <Video size={20} />
                    </div>
                    <label className="text-[12px] font-black text-red-900 uppercase tracking-widest leading-none block shrink-0">2. YouTube API Keys</label>
                    <div className="ml-auto flex flex-nowrap items-center justify-end gap-2 shrink-0">
                      <a href="https://console.cloud.google.com/apis/library/youtube.googleapis.com" target="_blank" rel="noopener noreferrer" className="inline-flex items-center justify-center gap-1 rounded-xl border border-red-200 bg-white px-2.5 py-2 text-[9px] font-bold text-red-600 shadow-sm hover:bg-red-100 transition-all active:scale-95 whitespace-nowrap shrink-0">
                        Lấy Key YouTube <ExternalLink size={11} />
                      </a>
                      <button type="button" onClick={openKeyGuideVideo} className="inline-flex items-center justify-center gap-1 rounded-xl border border-red-600 bg-red-600 px-2.5 py-2 text-[9px] font-bold text-white shadow-sm hover:bg-red-700 transition-all active:scale-95 whitespace-nowrap shrink-0">
                        <Play size={11} /> Hướng dẫn lấy Key
                      </button>
                    </div>
                  </div>
