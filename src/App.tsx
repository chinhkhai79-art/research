...
const UI_TRANSLATIONS_VI_TO_EN: Record<string, string> = {
  ...
  'Đăng xuất': 'Log out',
  'ĐĂNG NHẬP': 'LOG IN',
  'Đăng nhập Google': 'Log in with Google',
  'Nâng cấp': 'Upgrade',
  'Nâng cấp thêm': 'Upgrade more',
  'Nâng cấp gói': 'Upgrade plan',
  'NÂNG CẤP GÓI': 'UPGRADE PLAN',
  'LÀM MỚI': 'REFRESH',
  'Làm mới': 'Refresh',
  'Làm mới cài đặt & kết quả': 'Refresh settings & results',
  'TÌM KÊNH & ĐÁNH GIÁ TỪ KHÓA': 'FIND CHANNELS & RATE KEYWORDS',
  'Tìm kênh': 'Find channels',
  'PHÂN TÍCH ĐỐI THỦ (SPY)': 'COMPETITOR ANALYSIS (SPY)',
  'KIỂM TRA LINK VIDEO': 'CHECK VIDEO LINK',
  'THEO DÕI ĐỐI THỦ (TRACKING)': 'TRACK COMPETITORS',
  'TÌM NGÁCH YOUTUBE': 'YOUTUBE NICHE RESEARCH',
  'TỪ KHÓA': 'KEYWORD',
  'Từ khóa': 'Keyword',
  'KHU VỰC': 'REGION',
  'Khu vực': 'Region',
  'ĐĂNG TRONG': 'PUBLISHED IN',
  'Đăng trong': 'Published in',
  'Số lượng': 'Quantity',
  'SỐ LƯỢNG PHÂN TÍCH': 'ANALYSIS COUNT',
  'PHẠM VI SUB': 'SUBSCRIBER RANGE',
  'Tự động chuyển từ khóa cho đến khi đủ 10 Kênh': 'Auto switch keywords until 10 channels are found',
  'Săn kênh nhỏ / Mới Trend': 'Hunt small / new trending channels',
  'Deep Drill: dưới 50.000 sub, 30 ngày': 'Deep Drill: under 50,000 subs, 30 days',
  'BẮT ĐẦU SĂN KÊNH': 'START CHANNEL HUNT',
  'DANH SÁCH KÊNH QUÉT ĐƯỢC (TỰ ĐỘNG LỌC THEO ĐIỀU KIỆN)': 'SCANNED CHANNEL LIST (AUTO-FILTERED)',
  'TỔNG': 'TOTAL',
  ...
  'Bạn có chắc chắn muốn xóa toàn bộ lịch sử API Key không?': 'Are you sure you want to clear all API key history?',
  'Bạn có chắc chắn muốn xóa toàn bộ lịch sử Gemini API Key không?': 'Are you sure you want to clear all Gemini API key history?',
  ...
  'Check Gemini Key': 'Check Gemini Key',
  'Kiểm tra key': 'Check key',
  'Ẩn key': 'Hide key',
  'Hiện key': 'Show key',
  'Chưa có key': 'No key yet',
  'Keys': 'Keys',
  'Lỗi': 'Errors',
  'Không lỗi': 'No errors',
  'NICHE RESEARCH': 'NICHE RESEARCH',
  'TỪ KHÓA / NGÁCH RESEARCH': 'RESEARCH KEYWORD / NICHE',
  ...
  'Từ Khóa': 'Keywords',
  'Mẫu Thumbnail': 'Thumbnail Templates',
  'Lịch sử Nghiên cứu': 'Research History',
  'GỢI Ý NGÁCH & CHỦ ĐỀ KÊNH': 'NICHE & CHANNEL TOPIC IDEAS',
  ...
  'Thấp': 'Low',
  'Nguồn': 'Source',
  'Hiển thị tối đa 6 video': 'Show up to 6 videos',
  'Xem': 'Watch',
  'Copy từ khóa': 'Copy keyword',
  'DỮ LIỆU NGÁCH THEO KHU VỰC': 'NICHE DATA BY REGION',
  'CHỌN KHU VỰC ĐỂ ĐỔI NGÔN NGỮ CHỦ ĐỀ/KEY. BẤM KÍNH LÚP TỪNG CHỦ ĐỀ ĐỂ TÌM KEY ĐÚNG KHU VỰC; ƯU TIÊN 30 NGÀY, NẾU THIẾU DỮ LIỆU SẼ MỞ RỘNG TOÀN THỜI GIAN.': 'Choose a region to switch topic/key language. Click the magnifier for each topic to find region-matched keys; prioritize 30 days, expand to all time if data is limited.',
  'CẬP NHẬT': 'UPDATED',
  'LƯU TREND HOT ĐÃ QUÉT': 'SAVE SCANNED HOT TRENDS',
  'Đang lưu...': 'Saving...',
  'DANH SÁCH GỢI Ý NGÁCH LIÊN QUAN - TỪ KHÓA ĐỀ XUẤT': 'RELATED NICHE IDEAS - SUGGESTED KEYWORDS',
  'ẨN BẢNG': 'HIDE TABLE',
  'HIỆN BẢNG GỢI Ý NGÁCH': 'SHOW NICHE IDEAS',
  'TỪ KHÓA / NGÁCH TIỀM NĂNG': 'POTENTIAL KEYWORD / NICHE',
  'CẠNH TRANH': 'COMPETITION',
  'ĐIỂM SEO': 'SEO SCORE',
  'DÙNG': 'USE',
  'CẢNH BÁO LỖI KHI ĐANG QUÉT': 'SCAN ERROR WARNING',
  'Mẹo: Kiểm tra lại API Key xem có bị hết hạn hoặc sai cú pháp không. Hoặc thử đổi vùng quét (Region).': 'Tip: Check whether the API key expired or has the wrong format. Or try changing the scan region.',
  'DANH SÁCH': 'LIST',
  'CHỦ ĐỀ NGÁCH GỢI Ý': 'SUGGESTED NICHE TOPICS',
  'Tìm kiếm ngách...': 'Search niches...',
  ...
  'Danh sách các video của kênh này lọt top trending': 'Videos from this channel that entered top trending',
  'Người đăng': 'Uploader',
  'Bình luận': 'Comments',
...
function translateUiText(value: string, lang: UiLanguage): string {
  if (lang === 'vi') return value;
  const normalized = value.replace(/\s+/g, ' ').trim();
  if (!normalized) return value;
  if (UI_TRANSLATIONS_VI_TO_EN[normalized]) {
    return value.replace(normalized, UI_TRANSLATIONS_VI_TO_EN[normalized]);
  }

  let translated = value;
  const entries = Object.entries(UI_TRANSLATIONS_VI_TO_EN)
...
function UiLanguageBridge({ lang }: { lang: UiLanguage }) {
  useEffect(() => {
    const root = document.body;
    if (!root) return;
    root.setAttribute('data-ui-lang', lang);

    const ignoredTags = new Set(['SCRIPT', 'STYLE', 'NOSCRIPT', 'IFRAME', 'CODE', 'PRE']);
    const shouldSkip = (el: Element | null) => {
      if (!el) return false;
      if (ignoredTags.has(el.tagName)) return true;
      if ((el as HTMLElement).dataset?.noUiTranslate === 'true') return true;
      return false;
    };
    ...
    const translateNode = (node: Node) => {
      if (node.nodeType === Node.TEXT_NODE) {
        ...
        if (anyNode.__vtwOriginalText === undefined) anyNode.__vtwOriginalText = raw;
        const original = anyNode.__vtwOriginalText || raw;
        node.textContent = lang === 'vi' ? original : translateUiText(original, 'en');
        return;
      }
      ...
      ['title', 'placeholder', 'aria-label', 'alt'].forEach((attr) => {
        ...
        if (!current) return;
        const key = `uiOriginal${attr.replace(/-([a-z])/g, (_, c) => c.toUpperCase())}`;
        if (!el.dataset[key]) el.dataset[key] = current;
        const original = el.dataset[key] || current;
        el.setAttribute(attr, lang === 'vi' ? original : translateUiText(original, 'en'));
      });
    ...
    observer.observe(root, { childList: true, subtree: true, characterData: true, attributes: true, attributeFilter: ['title', 'placeholder', 'aria-label', 'alt'] });
...
function UiLanguageSwitch({ lang, onChange }: { lang: UiLanguage; onChange: (lang: UiLanguage) => void }) {
  return (
    <div className="vtw-language-switch flex items-center gap-1 bg-white border border-blue-100 rounded-xl px-2 py-1 shadow-sm shrink-0" data-no-ui-translate="true">
      <span className="text-[9px] font-black text-slate-400 uppercase hidden sm:inline">Ngôn ngữ</span>
      <select
        value={lang}
        onChange={(e) => onChange(e.target.value as UiLanguage)}
        className="bg-transparent text-[10px] font-black text-blue-700 outline-none cursor-pointer"
...
export default function App() {
  ...
  const [uiLanguage, setUiLanguage] = useState<UiLanguage>(() => {
    try {
      return (localStorage.getItem('vtw_ui_language') as UiLanguage) || 'vi';
    } catch {
      return 'vi';
    }
  });

  const changeUiLanguage = (nextLang: UiLanguage) => {
    setUiLanguage(nextLang);
    try { localStorage.setItem('vtw_ui_language', nextLang); } catch (_) {}
  };
...
- Với tags và bình luận, phải ưu tiên hiển thị/nhận xét từ dữ liệu YouTube API đã cung cấp.`;
  ...
  return (
    <div className="vtw-mobile-app min-h-screen bg-[#f4f4f4] text-[12px] font-[Tahoma,Arial,sans-serif] selection:bg-[#9fc8ff]" onClick={closeMenu}>
      <UiLanguageBridge lang={uiLanguage} />
      {/* Header */}
      <div className="vtw-app-header bg-white border-b border-[#ccc] px-3 py-1.5 shadow-sm">
        <div className="flex items-center justify-between gap-3 vtw-app-header-row">
          ...
          <div className="vtw-header-actions flex items-center gap-2 min-w-0 flex-1 justify-end">
            <UiLanguageSwitch lang={uiLanguage} onChange={changeUiLanguage} />
            {user ? (
