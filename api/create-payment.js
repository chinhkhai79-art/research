...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!plan) return res.status(400).json({ success:false, error:'Không tìm thấy gói thanh toán.' });
    ...
    const orderCode = String(pay.orderPrefix || 'RESEARCH').toUpperCase() + Date.now().toString() + Math.floor(Math.random() * 900 + 100);
    ...
    await db.collection('payments').doc(orderCode).set({
      app:'research', orderCode, planId, planName:plan.name, amount:plan.amount, days:plan.days,
      userId, userEmail, userPhone, userName, status:'pending', paid:false,
      bankId:pay.bankId, accountNo:pay.accountNo, accountName:pay.accountName, qrUrl, paymentUrl,
      createdAt:FieldValue.serverTimestamp(), updatedAt:FieldValue.serverTimestamp()
    });

    return res.status(200).json({
      success:true, orderCode, paymentCode:orderCode, planId, packageId:planId,
      planName:plan.name, packageName:plan.name, amount:plan.amount, days:plan.days,
      bankId:pay.bankId, accountNo:pay.accountNo, accountName:pay.accountName, qrUrl, paymentUrl, plans
    });
  } catch(e){
