...
export default async function handler(req, res) {
  ...
  try {
    ...
    result.checks.firestoreDatabaseId = process.env.FIRESTORE_DATABASE_ID || "(default)";
    ...
    const ref = db.collection("_debug").doc("research_connection_test");
    ...
    result.success = true;
    result.message = "Firebase Admin + Firestore đang hoạt động tốt.";
    ...
    if (msg.includes("5 NOT_FOUND") || String(error?.code).includes("5")) {
      result.fixes.push("Sai Firestore database ID hoặc chưa có database. Kiểm tra FIRESTORE_DATABASE_ID.");
    }
