...
export default async function handler(req, res) {
  ...
  if (route === 'overview' || route === 'account-overview') {
    req.query.overview = '1';
  ...
  return res.status(400).json({
    success: false,
    error: 'Thiếu action admin-account. Dùng action=users, overview, activate, disable, logs, revenue, orders hoặc reset-history.'
  });
}
