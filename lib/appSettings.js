...
export function normalizeSettings(input={}){
  ...
  // Giá gói được quản lý cố định trong mã nguồn để dữ liệu cũ trên Firestore
  // không thể ghi đè và làm trang thanh toán hiển thị sai giá.
  const plans = {};
  for (const id of ['1m','3m','6m','12m']) {
    const x = DEFAULT_SETTINGS.payment.plans[id];
    plans[id] = { ...x };
  }
  return {
    payment: {
