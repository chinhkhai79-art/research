...
function normalizePaymentSuccessTemplate(source={}){
  ...
  const fallback = DEFAULT_SETTINGS.emailTemplates.paymentSuccess;
  return {
...
export function diffSettings(oldS, newS) {
  ...
  if (oldTemplates !== newTemplates) {
    changes.push({ field: 'emailTemplates.paymentSuccess', oldValue: '(mẫu cũ)', newValue: '(mẫu mới)' });
  }
  return changes;
}
