...
function cleanBase(v){
  ...
  if (!raw || /^(https?:\/\/)?research\.vanthemmo\.com$/i.test(raw)) return CANONICAL_BASE_URL;
  try {
    const u = new URL(raw.startsWith('http') ? raw : `https://${raw}`);
    if (u.hostname.toLowerCase() === 'research.vanthemmo.com') return CANONICAL_BASE_URL;
    return u.origin;
  } catch {
    return raw;
  }
...
export function normalizeSettings(input={}){
  ...
  const bankAccount = cleanAccount(p.bankAccount || p.accountNo || p.accountNumber || p.account || DEFAULT_SETTINGS.payment.bankAccount);
