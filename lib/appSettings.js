...
export const DEFAULT_KEY_GUIDE_URL = 'https://drive.google.com/file/d/1CxCVSqGqH9ijg0rRBdTJciYoV5uABYW5/view?usp=drive_link';
...
export const DEFAULT_SETTINGS = {
  ...
  smtp: {
    ...
    smtpDailyLimitEnabled: true,
    smtpDailyLimit: 300,
    smtpSendDelayEnabled: true,
    smtpSendDelaySeconds: 10
  },
  mailRuntime: {
    date: '',
    sentToday: 0,
  ...
  account: {
    ...
    trialYoutubeApiKey: '',
    trialGeminiApiKey: '',
    trialIpLimitEnabled: false,
    trialIpMaxActive: 20,
    trialIpTtlMinutes: 10
  },
  trialIpRuntime: {
    activeIps: {},
    lastUpdatedAt: ''
  ...
  emailTemplates: {
    paymentSuccess: {
      ...
      content: {
        ...
        keyGuideUrl: DEFAULT_KEY_GUIDE_URL,
      ...
      html: [
        ...
        '</table>',
        '<p style="text-align:center;margin:26px 0 10px"><a href="{{toolUrl}}" style="display:inline-block;background:#ff5a1f;color:#ffffff;text-decoration:none;padding:13px 22px;border-radius:12px;font-weight:bold;margin:4px 6px">Truy cập công cụ</a><a href="'+DEFAULT_KEY_GUIDE_URL+'" style="display:inline-block;background:#0f3b66;color:#ffffff;text-decoration:none;padding:13px 22px;border-radius:12px;font-weight:bold;margin:4px 6px">Hướng dẫn lấy Key</a></p>',
...
function normalizeBankCode(code){
  const c = str(code).toUpperCase().replace(/[^A-Z0-9]/g,'');
  if (!c) return '';
  if (c === 'MBBANK') return 'MB';
  if (c === 'VIETCOMBANK') return 'VCB';
  if (c === 'TECHCOMBANK') return 'TCB';
  if (c === 'VIETINBANK') return 'CTG';
  return c;
}
function bankBin(code, fallback=''){
  const c = normalizeBankCode(code);
  if (/^\d{6}$/.test(c)) return c;
  return BANK_BIN[c] || fallback || '';
}
function resolveBankIdentity(paymentSource={}, mergedPayment={}){
  // Bank code/name is the source of truth. A stale bankId from the previous bank
  // must never override a newly selected bank (e.g. MB + old ACB BIN 970416).
  const selectedRaw = str(
    paymentSource.bankCode || paymentSource.bankName || paymentSource.bank ||
    mergedPayment.bankCode || mergedPayment.bankName || mergedPayment.bank || ''
  );
  const selectedCode = normalizeBankCode(selectedRaw);
  const selectedBin = bankBin(selectedCode);
  const storedBin = bankBin(
    paymentSource.bankId || paymentSource.bankBin || mergedPayment.bankId || mergedPayment.bankBin || ''
  );
  const fallbackCode = normalizeBankCode(DEFAULT_SETTINGS.payment.bankCode || DEFAULT_SETTINGS.payment.bankName);
  const bankCode = selectedCode && BANK_BIN[selectedCode] ? selectedCode : (selectedCode || fallbackCode);
  const bankId = selectedBin || storedBin || bankBin(fallbackCode, DEFAULT_SETTINGS.payment.bankId);
  return { bankCode, bankName: bankCode, bankId };
}
function boolSetting(v, fallback=true){
  if (v === undefined || v === null || v === '') return fallback;
  if (typeof v === 'boolean') return v;
  const raw = String(v).trim().toLowerCase();
  if (['0','false','off','no','khong','không'].includes(raw)) return false;
  if (['1','true','on','yes','co','có'].includes(raw)) return true;
  return fallback;
}
...
function normalizeKeyListText(value){
  if (Array.isArray(value)) {
    return Array.from(new Set(value.map(v => str(v).trim()).filter(Boolean))).join('\n');
  }
  const raw = str(value || '');
  return Array.from(new Set(raw.split(/[\r\n,;]+/).map(v => str(v).trim()).filter(Boolean))).join('\n');
}

function normalizeMailRuntime(source={}){
  const raw = source && typeof source === 'object' ? source : {};
  return {
    date: str(raw.date || raw.day || ''),
    sentToday: clampNumber(raw.sentToday ?? raw.count, 0, 0, 1000000),
    lastSentAt: str(raw.lastSentAt || raw.last_sent_at || '')
  };
}
...
function normalizeAccount(source={}){
  ...
  return {
    ...
    trialHours,
    trialLabel,
    trialYoutubeApiKey: normalizeKeyListText(rawTrial.trialYoutubeApiKey || rawTrial.trialYoutubeApiKeys || rawTrial.youtubeApiKey || rawTrial.youtubeApiKeys || rawTrial.youtubeApiKeyV3 || rawTrial.ytApiKey || ''),
    trialGeminiApiKey: normalizeKeyListText(rawTrial.trialGeminiApiKey || rawTrial.trialGeminiApiKeys || rawTrial.geminiApiKey || rawTrial.geminiApiKeys || rawTrial.geminiKey || ''),
    trialIpLimitEnabled: boolSetting(rawTrial.trialIpLimitEnabled ?? rawTrial.ipLimitEnabled ?? rawTrial.activeIpLimitEnabled, false),
    trialIpMaxActive: 20,
    trialIpTtlMinutes: 10
  };
}

function normalizeTrialIpRuntime(source={}){
  const raw = source.trialIpRuntime && typeof source.trialIpRuntime === 'object' ? source.trialIpRuntime : {};
  const rawIps = raw.activeIps && typeof raw.activeIps === 'object' ? raw.activeIps : {};
  const activeIps = {};
  const now = Date.now();
  const ttlMs = 10 * 60 * 1000;
  for (const [ip, ts] of Object.entries(rawIps)) {
    const cleanIp = str(ip).slice(0, 96);
    const n = Number(ts);
...
function normalizeGuide(source={}){
  const rawGuide = source.guide && typeof source.guide === 'object' ? source.guide : {};
  const rawAccount = source.account && typeof source.account === 'object' ? source.account : {};
  const videoUrl = str(rawGuide.videoUrl || rawGuide.youtubeUrl || rawGuide.url || rawAccount.guideVideoUrl || '');
  const keyGuideVideoUrl = str(rawGuide.keyGuideVideoUrl || rawGuide.apiKeyGuideVideoUrl || rawGuide.keyVideoUrl || rawAccount.keyGuideVideoUrl || '');
  return { videoUrl, keyGuideVideoUrl };
}
...
function normalizeEmailTemplate(source={}, fallbackKey='paymentSuccess'){
  ...
  if (fallbackKey === 'paymentSuccess') {
    normalizedContent.keyGuideUrl = str(rawContent.keyGuideUrl || rawContent.guideKeyUrl || rawContent.keyGuideLink || rawContent.guideLink || fallback.content?.keyGuideUrl || DEFAULT_KEY_GUIDE_URL);
  }
  return {
...
export function normalizeSettings(input={}){
  ...
  const p = raw.payment || {};
  const s = raw.smtp || {};
  const bankIdentity = resolveBankIdentity(sourcePayment, p);
  const bankName = bankIdentity.bankName;
  const bankId = bankIdentity.bankId;
  ...
  return {
    ...
    smtp: {
      ...
      smtpDailyLimitEnabled: boolSetting(s.smtpDailyLimitEnabled ?? s.dailyLimitEnabled, true),
      smtpDailyLimit: 300,
      smtpSendDelayEnabled: boolSetting(s.smtpSendDelayEnabled ?? s.sendDelayEnabled, true),
      smtpSendDelaySeconds: clampNumber(s.smtpSendDelaySeconds ?? s.sendDelaySeconds, 10, 10, 60)
    },
    account: normalizeAccount(raw),
    guide: normalizeGuide(raw),
    emailTemplates: normalizeEmailTemplates(raw.emailTemplates || raw.email_templates || raw.mailTemplates || raw.email || {}),
    mailRuntime: normalizeMailRuntime(raw.mailRuntime || raw.mail_runtime || {}),
    trialIpRuntime: normalizeTrialIpRuntime(raw)
  };
}
...
export async function saveAppSettings(input={}){
  ...
  // Riêng key dùng thử cho tài khoản trial cần cho phép admin xóa trắng thủ công.
  if (Object.prototype.hasOwnProperty.call(input?.account || {}, 'trialYoutubeApiKey') || Object.prototype.hasOwnProperty.call(input?.account || {}, 'trialYoutubeApiKeys')) {
    merged.account = { ...(merged.account || {}), trialYoutubeApiKey: normalizeKeyListText(input.account.trialYoutubeApiKey || input.account.trialYoutubeApiKeys || '') };
  }
  if (Object.prototype.hasOwnProperty.call(input?.account || {}, 'trialGeminiApiKey') || Object.prototype.hasOwnProperty.call(input?.account || {}, 'trialGeminiApiKeys')) {
    merged.account = { ...(merged.account || {}), trialGeminiApiKey: normalizeKeyListText(input.account.trialGeminiApiKey || input.account.trialGeminiApiKeys || '') };
  }
  // Cho phép admin xóa trắng các link video hướng dẫn khi cần.
  if (Object.prototype.hasOwnProperty.call(input?.guide || {}, 'videoUrl')) {
    merged.guide = { ...(merged.guide || {}), videoUrl: String(input.guide.videoUrl || '').trim() };
  }
  if (Object.prototype.hasOwnProperty.call(input?.guide || {}, 'keyGuideVideoUrl')) {
    merged.guide = { ...(merged.guide || {}), keyGuideVideoUrl: String(input.guide.keyGuideVideoUrl || '').trim() };
  }
  // Runtime IP dùng thử là dữ liệu hệ thống, giữ nguyên khi admin lưu cấu hình khác.
  if (current.trialIpRuntime && !Object.prototype.hasOwnProperty.call(input || {}, 'trialIpRuntime')) {
    merged.trialIpRuntime = current.trialIpRuntime;
  }
...
export function maskSettings(settings){
  ...
  const activeIpCount = Object.keys(s.trialIpRuntime?.activeIps || {}).length;
  // Theo yêu cầu admin: API Key SePay và App Password Gmail luôn hiện lại trên trang cấu hình sau khi lưu.
  // Runtime IP chỉ trả số lượng, không lộ danh sách IP ra public payment-config.
  return {
    ...s,
    trialIpRuntime: { activeCount: activeIpCount, lastUpdatedAt: s.trialIpRuntime?.lastUpdatedAt || '' },
    smtp: { ...s.smtp, smtpPass: s.smtp.smtpPass || '', hasPass: Boolean(s.smtp.smtpPass) }
  };
}
...
const TRACKED_FIELDS = {
  ...
  smtp: ['smtpUser', 'smtpHost', 'smtpPort', 'smtpSecure', 'senderName', 'emailBaseUrl', 'smtpDailyLimitEnabled', 'smtpDailyLimit', 'smtpSendDelayEnabled', 'smtpSendDelaySeconds'],
  account: ['trialDurationValue', 'trialDurationUnit', 'trialIpLimitEnabled', 'trialIpMaxActive', 'trialIpTtlMinutes'],
  guide: ['videoUrl', 'keyGuideVideoUrl']
};
const SECRET_FIELDS = new Set(['sepaySecret', 'smtpPass', 'trialYoutubeApiKey', 'trialGeminiApiKey']);
...
export function diffSettings(oldS, newS) {
  ...
  for (const f of SECRET_FIELDS) {
    const section = f === 'sepaySecret' ? 'payment' : (f === 'smtpPass' ? 'smtp' : 'account');
    const oldV = String(oldS?.[section]?.[f] ?? '');
    const newV = String(newS?.[section]?.[f] ?? '');
    if (oldV !== newV) {
      changes.push({
