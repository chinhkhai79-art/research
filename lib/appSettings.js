...
const LEGACY_DEFAULT_BANK_ACCOUNTS = new Set([['1313','1447'].join('')]);
const LEGACY_DEFAULT_BANK_OWNERS = new Set([['LE','VAN','KHAI'].join(' ')]);
...
export function normalizeSettings(input={}){
  ...
  const bankAccount = LEGACY_DEFAULT_BANK_ACCOUNTS.has(requestedBankAccount)
    ? DEFAULT_SETTINGS.payment.bankAccount
    : (requestedBankAccount || DEFAULT_SETTINGS.payment.bankAccount);
  ...
  const bankOwner = (LEGACY_DEFAULT_BANK_ACCOUNTS.has(requestedBankAccount) || (bankAccount === DEFAULT_SETTINGS.payment.bankAccount && LEGACY_DEFAULT_BANK_OWNERS.has(requestedBankOwner)))
    ? DEFAULT_SETTINGS.payment.bankOwner
    : (requestedBankOwner || DEFAULT_SETTINGS.payment.bankOwner);
  ...
  // Chỉ dùng giá trong Firestore khi cấu hình đã được lưu từ trang quản trị mới.
