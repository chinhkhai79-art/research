...
function normalizeAccount(source={}){
  ...
  return {
    ...
    trialHours,
    trialLabel,
    trialGeminiApiKey: str(rawTrial.trialGeminiApiKey || rawTrial.geminiApiKey || ''),
    trialYoutubeApiKey: str(rawTrial.trialYoutubeApiKey || rawTrial.youtubeApiKey || '')
  };
}
...
export async function saveAppSettings(input={}){
  ...
  // Hai key dùng thử cho phép admin chủ động xóa bằng chuỗi rỗng.
  // deepMerge bỏ qua chuỗi rỗng nên cần áp lại giá trị khi field thực sự có trong payload.
  if (Object.prototype.hasOwnProperty.call(input.account || {}, 'trialGeminiApiKey')) {
    merged.account.trialGeminiApiKey = str(input.account.trialGeminiApiKey);
  }
  if (Object.prototype.hasOwnProperty.call(input.account || {}, 'trialYoutubeApiKey')) {
    merged.account.trialYoutubeApiKey = str(input.account.trialYoutubeApiKey);
  }
...
const SECRET_FIELDS = [
  { section: 'payment', field: 'sepaySecret' },
  { section: 'smtp', field: 'smtpPass' },
  { section: 'account', field: 'trialGeminiApiKey' },
  { section: 'account', field: 'trialYoutubeApiKey' }
];
...
export function diffSettings(oldS, newS) {
  ...
  for (const { section, field: f } of SECRET_FIELDS) {
    const oldV = String(oldS?.[section]?.[f] ?? '');
    const newV = String(newS?.[section]?.[f] ?? '');
    if (oldV !== newV) {
      changes.push({
