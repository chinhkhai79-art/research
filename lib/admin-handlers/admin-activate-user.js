...
const PLAN_MAP = {
  'trial_settings': { planId: 'trial_settings', planName: 'KÍCH HOẠT DÙNG THỬ', days: 0, trial: true },
...
async function activateTrialSupabaseUser({ uidInput, email, body, reason }) {
  ...
  const now = new Date();
  const oldTrialExpiresAt = toDate(current.trialExpiresAt);
  const baseDate = oldTrialExpiresAt && oldTrialExpiresAt.getTime() > now.getTime() ? oldTrialExpiresAt : now;
  const trialExpiresAt = addHours(baseDate, trialPlan.hours);
  const data = {
    ...current,
    userId: uid,
    uid,
    ...
    // Không giữ hạn PRO cũ trong raw để giao diện trial ưu tiên đúng hạn dùng thử.
    ...
    lastAdminAction: 'activate_trial',
    lastAdminReason: reason,
    lastAdminTrialHours: trialPlan.hours,
  ...
  await sbAddAdminLog({
    ...
    planId: trialPlan.planId,
    planName: trialPlan.planName,
    trialHours: trialPlan.hours,
    oldTrialExpiresAt: oldTrialExpiresAt ? oldTrialExpiresAt.toISOString() : null,
    newTrialExpiresAt: trialExpiresAt.toISOString(),
    reason
  });
  return { uid, email: data.email, expiresAt: trialExpiresAt, trialPlan, matchedExisting: found.exists, manual: Boolean(found.manual && !found.exists) };
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (action === 'activate_trial' || String(body.planId || body.plan || '').trim() === 'trial_settings') {
      const uidInput = String(body.uid || body.userId || '').trim();
      const email = String(body.email || '').trim().toLowerCase();
      const reason = String(body.reason || '').trim() || 'Kích hoạt dùng thử thủ công theo cấu hình trial hiện tại';
      if (!uidInput && !email) return res.status(400).json({ success: false, error: 'Cần nhập UID hoặc email.' });
      const result = await activateTrialSupabaseUser({ uidInput, email, body, reason });
      return res.status(200).json({
        success: true,
        message: `Đã kích hoạt dùng thử ${result.trialPlan.value} ${result.trialPlan.unit === 'days' ? 'ngày' : 'giờ'}.`,
        uid: result.uid,
        email: result.email,
        planId: result.trialPlan.planId,
        planName: result.trialPlan.planName,
        trialHours: result.trialPlan.hours,
    ...
    const exactPlan = PLAN_MAP[planKey] || PLAN_MAP[String(body.days || '')];
    const selected = exactPlan || PLAN_MAP['90'];
    const requestedDays = Number(body.days || selected.days || 90);
    if (!Number.isFinite(requestedDays) || requestedDays < 1 || requestedDays > 3650) {
      return res.status(400).json({ success: false, error: 'Số ngày PRO phải từ 1 đến 3650.' });
    }
    const days = Math.round(requestedDays);
    const isCustomDuration = !PLAN_MAP[planKey] || days !== Number(selected.days);
    const planId = isCustomDuration ? `manual_${days}d` : selected.planId;
