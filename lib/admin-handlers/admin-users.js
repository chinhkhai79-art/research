...
import { isSupabaseConfigured, listUsers as sbListUsers, listUsersForBulk as sbListUsersForBulk } from '../supabaseAdmin.js';
...
function overviewMatch(user, q) {
  const term = String(q || '').trim().toLowerCase();
  if (!term) return true;
  const hay = [
    user.email, user.name, user.uid, user.status, user.accountType,
    user.planId, user.planName, user.remainingText, user.lastPaymentOrderCode
...
function isManualUserRecord(user) {
  return String(user?.uid || user?.userId || '').trim().toLowerCase().startsWith('manual_');
}

function removeDuplicateManualUsers(users = []) {
  const list = Array.isArray(users) ? users.filter(Boolean) : [];
  const realUids = new Set();
  const realEmails = new Set();

  for (const user of list) {
    if (isManualUserRecord(user)) continue;
    const uid = String(user.uid || user.userId || '').trim();
    const email = normalizedEmail(user.email);
    if (uid) realUids.add(uid);
    if (email) realEmails.add(email);
  }

  return list.filter(user => {
    if (!isManualUserRecord(user)) return true;
    const email = normalizedEmail(user.email);
    const migratedTo = String(user.migratedTo || '').trim();
    // Chỉ ẩn bản ghi manual khi đã có tài khoản thật tương ứng.
    // Manual chưa có tài khoản thật vẫn được giữ để admin quản lý/kích hoạt.
    if (email && realEmails.has(email)) return false;
    if (migratedTo && realUids.has(migratedTo)) return false;
    return true;
  });
}

function summarizeUsers(users = []) {
  const out = { total: users.length, pro: 0, trial: 0, expired: 0, unchecked: 0 };
  for (const u of users) {
    const status = String(u.status || '').toUpperCase();
    if (status === 'PRO') out.pro += 1;
    else if (status === 'TRIAL') out.trial += 1;
    else if (status === 'CHƯA KIỂM TRA') out.unchecked += 1;
    else out.expired += 1;
  }
  return out;
}
...
async function handleOverview(req, res) {
  ...
  const statusFilter = String(req.query.status || req.body?.status || 'all').trim().toUpperCase();
  ...
  const cacheKey = `overview:${q || '_all'}:${statusFilter}:${limit}:${page}`;
  ...
  // Loại bản ghi manual trùng email/đã migrate trước khi tính tổng và phân trang.
  // Nhờ đó Tổng tài khoản / PRO / TRIAL / HẾT HẠN đều không bị đếm trùng.
  const dedupedUsers = removeDuplicateManualUsers(rows.map(normalizeSupabaseUser));
  const allUsers = dedupedUsers.filter(u => overviewMatch(u, q));
  const summary = summarizeUsers(allUsers);
  let filtered = allUsers;
  if (statusFilter && statusFilter !== 'ALL') {
    filtered = allUsers.filter(u => String(u.status || '').toUpperCase() === statusFilter);
  }
  ...
  const response = {
    ...
    nextPage: offset + limit < filtered.length ? page + 1 : null,
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (String(req.query.overview || req.body?.overview || '') === '1') return await handleOverview(req, res);
    ...
    const normalizedUsers = rows.map(normalizeSupabaseUser);
    // Danh sách thường cũng không hiện bản ghi manual đã được gộp vào UID thật.
    const users = normalizedUsers.filter(u => !(isManualUserRecord(u) && String(u.migratedTo || '').trim()));
