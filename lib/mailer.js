...
export function buildPaymentSuccessEmail({ email, userName, orderCode, planName, amount, paidAt, expiresAt, toolUrl, settings }){
  ...
  const vars = {
    name: userName || defaultNameFromEmail(email),
    email: email || '',
    orderCode: orderCode || '',
  ...
  const subject = renderTemplate(tmpl.subject || 'Thanh toán thành công - {{planName}}', vars) || 'Thanh toán thành công - Tài khoản đã nâng cấp PRO';
